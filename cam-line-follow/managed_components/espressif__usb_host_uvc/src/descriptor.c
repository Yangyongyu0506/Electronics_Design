...
int parse_configuration(libusb_config_descriptor_t *config, const uint8_t *buffer, int size)
{
    ...
    for (int i = 0; i < config->bNumInterfaces; i++) {
        ...
        for (int alt = 0; alt < alt_interfaces; alt++) {
            ...
            if (ep_desc != NULL) {
                // Class-specific descriptors following the last endpoint (e.g. UVC
                // VideoStreaming descriptors) belong to the interface itself.
                set_extra_data(&extra, &altsetting->extra, &altsetting->extra_length,
                               ((uint8_t *)ep_desc) + ep_desc->bLength);
            }
