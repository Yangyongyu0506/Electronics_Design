...
static const char *TAG = "line-ball";
...
static void line_ctrl_task(void *arg)
{
    ...
    while (1) {
        ...
        if (s_ball_mode_active || s_avoid_active) {
            last_err = 0.0f;
            last_t_us = 0;
...
static void detect_mjpeg(const uint8_t *jpeg, size_t jpeg_len)
{
    ...
    for (uint32_t y = 0; y < image.height; ++y) {
        for (uint32_t x = 0; x < image.width; ++x) {
            ...
            if (y >= y_start && y < y_end && is_black_pixel(r, g, b)) {
    ...
    const uint32_t min_pixels = detection_target == BALL_GREEN
        ? (uint32_t)ceilf((float)image.width * image.height *
                          GREEN_DETECT_RATIO_PERCENT / 100.0f)
        : BALL_MIN_PIXELS;
    if (count[detection_target] >= min_pixels) {
        int32_t cx = (int32_t)(sum_x[detection_target] * s_decode_scale_divisor / count[detection_target]);
        int32_t cy = (int32_t)(sum_y[detection_target] * s_decode_scale_divisor / count[detection_target]);
        if (BALL_CAMERA_FLIPPED) {
            cx = (int32_t)s_frame_width - 1 - cx;
            cy = (int32_t)s_frame_height - 1 - cy;
        }
        portENTER_CRITICAL(&s_ball_mux);
        s_balls[detection_target].cx = cx;
        s_balls[detection_target].cy = cy;
        s_balls[detection_target].pixels = count[detection_target];
        s_balls[detection_target].last_seen_us = esp_timer_get_time();
        portEXIT_CRITICAL(&s_ball_mux);
        if (log_now) {
            ESP_LOGI(TAG, "%s detected: pixels=%u center=(%ld,%ld)",
                     ball_name(detection_target),
                     (unsigned)count[detection_target], (long)cx, (long)cy);
        }
    }
...
static void frame_processing_task(void *arg)
{
    ...
    while (true) {
        ...
        if (slot->len <= JPEG_FRAME_MAX_BYTES &&
            ...
            detect_mjpeg(slot->data, slot->len);
        } else {
