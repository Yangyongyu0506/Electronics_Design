...
static uint16_t get_endpoint_mps(const usb_config_desc_t *config_desc, uint8_t bEndpointAddress)
{
    int offset = 0;
    size_t total_length = config_desc->wTotalLength;
    const usb_standard_desc_t *next_desc = (const usb_standard_desc_t *)config_desc;
...
int libusb_submit_transfer(struct libusb_transfer *libusb_trans)
{
    ...
    // Make in transfer rounded up to MPS of its own endpoint.
    // ISO transfers should be effected by this, as there are supposed to be 512 bytes long
    if (is_in_endpoint(libusb_trans->endpoint)) {
        uint16_t mps = device->endpoint_mps;
        const usb_config_desc_t *config_desc = NULL;
        if (usb_host_get_active_config_descriptor(device->handle, &config_desc) == ESP_OK) {
            mps = get_endpoint_mps(config_desc, libusb_trans->endpoint);
        }
        length = usb_round_up_to_mps(length, mps);
    }
                                       ...
                                       int length)
{
    ...
    if (usb_host_get_device_descriptor(device->handle, &device_desc) != ESP_OK ||
        usb_host_device_info(device->handle, &dev_info) != ESP_OK) {
        return 0;
    }

    const usb_str_desc_t *str_desc = NULL;
    if (desc_index == device_desc->iManufacturer) {
        str_desc = dev_info.str_desc_manufacturer;
    } else if (desc_index == device_desc->iProduct) {
        str_desc = dev_info.str_desc_product;
    } else if (desc_index == device_desc->iSerialNumber) {
        str_desc = dev_info.str_desc_serial_num;
    }

    if (str_desc == NULL || length <= 0) {
        return 0;
    }

    int wlen = (str_desc->bLength - USB_STANDARD_DESC_SIZE) / 2;
    int n = 0;
    for (int i = 0; i < wlen && n < length - 1; i++) {
        data[n++] = (unsigned char)(str_desc->wData[i] & 0xFF);
    }
    data[n] = '\0';
    return n;
}
